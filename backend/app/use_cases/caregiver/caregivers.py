...
from app.schemas.caregiver_schema import CaregiverCreate, CaregiverResponse
...
def list_caregivers(skip: int = 0, limit: int = 20, db: Session = Depends(get_db)):
...
def update_caregiver(caregiver_id: int, dados: CaregiverCreate, db: Session = Depends(get_db)):
    caregiver = db.query(Caregiver).filter(Caregiver.id == caregiver_id).first()
    if not caregiver:
        raise HTTPException(status_code=404, detail="Cuidador não encontrado")
